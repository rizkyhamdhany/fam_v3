...
class master_zonasi_m extends CI_Model {
   public function getIdMax() { //query untuk mendapatkan id_kyw selanjutnya
        $sql = "select Raw_ID from Mst_Zonasi";
        ...
        if ($jml == 0) {
            ...
            $sql = "select max(right(Raw_ID,6)) as Raw_ID from Mst_Zonasi";
            $query = $this->db->query($sql);
            $hasil = $query->result();
   ...
   public function getzones() { //query untuk mendapatkan id_kyw selanjutnya
        $sql = "select ZoneID from Mst_Zonasi";
        $query = $this->db->query($sql);
        $jml = $query->num_rows();
        if ($jml == 0) {
            $ZoneID = "001";
            ...
            $sql = "select max(right(ZoneID,6)) as ZoneID from Mst_Zonasi";
            $query = $this->db->query($sql);
            $hasil = $query->result();
