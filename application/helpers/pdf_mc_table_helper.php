...
class pdf_mc_table extends FPDF{
	...
	function __construct($init_parameter) {
		//echo "xx";
		$this->SetTitle($init_parameter['title1']);
	...
	function Row($data)
	{
		...
		$nb=isset($this->height) ? $this->height : 0;
		// $nb=0;
		for($i=0;$i<count($data);$i++)
			$nb=max($nb,$this->NbLines($this->widths[$i],$data[$i]));
		for($i=0;$i<count($data);$i++)
			$nc=min($nb,$this->NbLines($this->widths[$i],$data[$i]));
		$h=5*$nb;
		...
		for($i=0;$i<count($data);$i++)
		{
			...
			$dy=($h-$maxrows*$this->FontSize)/2;
			// $this->SetY(isset($this->valign) ? $this->valign : $y);
			$ac = isset($this->valign) ? $this->valign : 0; 
			if($ac != 0){
				$this->SetY($y+$nc+$ac);
				$this->SetX($x);
			}
			//Draw the border
			$q = (isset($this->mergecell[$i]) ? $this->mergecell[$i] : 0);
			if($data[$i] != '~'){
			$this->Rect($x,$y,$w,$h+$q);
			//Print the text
	...
	function Footer(){
		...
		$this->SetFont('Arial','I',8);
		$this->Cell(0,10,'Dicetak pada tanggal '.date("d M Y"),0,0,'R');
	}
