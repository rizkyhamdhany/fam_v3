...
class master_cabang extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('master_cabang_m');
    ...
    function home() {
        ...
        if ($act==1) {
            ...
            $this->template->load('template/template_dataTable', 'master_cabang_v', $data);
        }
    } else {
      $data['multilevel'] = $this->user_m->get_data(0, $this->session->userdata('usergroup'));
            $data['menu_all'] = $this->user_m->get_menu_all(0);
...
//        print_r($requestData);die();
        ...
        $sql = "SELECT * from ams_cabang where Status like '%".$iStatus."%'";            
        $totalData = $this->global_m->tampil_semua_array($sql)->num_rows(); 
        $totalFiltered = $totalData;

        if (!empty($requestData['search']['value'])) {
            if ($iSearch=='1'){
                $sql = "SELECT * from ams_cabang where Status like '%".$iStatus."%'and BranchID like '%".$requestData['search']['value']."%'";
            }else if ($iSearch=='2'){
                $sql = "SELECT * from ams_cabang where Status like '%".$iStatus."%'and BranchCode like '%".$requestData['search']['value']."%'";
            }else if ($iSearch=='3'){
                $sql = "SELECT * from ams_cabang where Status like '%".$iStatus."%'and BranchName like '%".$requestData['search']['value']."%'";
            }else if ($iSearch=='4'){
                $sql = "SELECT * from ams_cabang where Status like '%".$iStatus."%'and BranchAlias like '%".$requestData['search']['value']."%'"; 
            }else if ($iSearch=='5'){
                $sql = "SELECT * from ams_cabang where Status like '%".$iStatus."%'and ZoneID like '%".$requestData['search']['value']."%'";
            }else{
                $sql = "SELECT * from ams_cabang where Status like '%".$iStatus."%'"; 
                ...
                $sql .= "or BranchAlias  like '%".$requestData['search']['value']."%'";
    ...
    public function ajax_UpdateStatusCategory(){
        ...
        $id = trim(element('BranchID',$i_list));
        ...
        $model = $this->global_m->ubah('ams_cabang', $data,'BranchID',$id);
        if ($model) {
            $notifikasi = Array(
                'msgType' => true,
                'msgTitle' => 'Success',
    ...
    public function ajax_UpdateCategory(){
        ...
        $BranchCode = trim(element('BranchCode',$i_list));
        $BranchName = trim(element('BranchName',$i_list)); 
        $BranchAlias = trim(element('BranchAlias',$i_list));
        $ZoneID = element('ZoneID',$i_list);

       
        $iStatus = trim(element('Status',$i_list));
        
        if(element('BranchID',$i_list)=="Generate"){
            $id = $this->master_cabang_m->getIdMax();
        ...
        if(element('BranchID',$i_list)=="Generate"){
        $model = $this->global_m->simpan('ams_cabang', $data);
       }else{
        $model = $this->global_m->ubah('ams_cabang', $data,'BranchID',$id);
       }
        if ($model) {
   ...
   public function getUserInfo() {
        ...
        $rows = $this->master_cabang_m->getUserInfo();
        $data['data'] = array();
        foreach ($rows as $row) {

            $array = array(
                                 
               'BranchID' => trim($roq->BranchID),    
               'BranchCode' => trim($row->BranchCode),
               'BranchName' => trim($row->BranchName),
               'BranchAlias' => trim($roq->BranchAlias),    
