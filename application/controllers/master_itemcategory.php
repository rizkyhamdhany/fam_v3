...
class master_itemcategory extends CI_Controller {
    ...
    function home() {
        ...
        $menuId = $this->home_m->get_menu_id('master_itemcategory/home');
...
//        print_r($requestData);die();
        ...
        $sql = "SELECT * from ams_itemcategory where Status like '%".$iStatus."%'";            
        $totalData = $this->global_m->tampil_semua_array($sql)->num_rows(); 
        $totalFiltered = $totalData;

        if (!empty($requestData['search']['value'])) {
            if ($iSearch=='1'){
                $sql = "SELECT * from ams_itemcategory where Status like '%".$iStatus."%'and ClassCode like '%".$requestData['search']['value']."%'";
            }else if ($iSearch=='2'){
                $sql = "SELECT * from ams_itemcategory where Status like '%".$iStatus."%'and IClassName like '%".$requestData['search']['value']."%'";
            }else{
                $sql = "SELECT * from ams_itemcategory where Status like '%".$iStatus."%'"; 
