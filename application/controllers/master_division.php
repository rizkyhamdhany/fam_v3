...
class master_division extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('master_division_m');
    ...
    function home() {
        ...
        $menuId = $this->home_m->get_menu_id('master_division/home');
        ...
        if ($act==1) {
            ...
            $this->template->set('title', 'Master Division');
            $this->template->load('template/template_dataTable', 'master_division_v', $data);
        }
    } else {
      $data['multilevel'] = $this->user_m->get_data(0, $this->session->userdata('usergroup'));
            $data['menu_all'] = $this->user_m->get_menu_all(0);
...
//        print_r($requestData);die();
        ...
        $sql = "SELECT * from Mst_Division where Status like '%".$iStatus."%'";            
        $totalData = $this->global_m->tampil_semua_array($sql)->num_rows(); 
        $totalFiltered = $totalData;

        if (!empty($requestData['search']['value'])) {
            if ($iSearch=='1'){
                $sql = "SELECT * from Mst_Division where Status like '%".$iStatus."%'and DivisionCode like '%".$requestData['search']['value']."%'";
            }else if ($iSearch=='2'){
                $sql = "SELECT * from Mst_Division where Status like '%".$iStatus."%'and DivisionName like '%".$requestData['search']['value']."%'";
            }else{
                $sql = "SELECT * from Mst_Division where Status like '%".$iStatus."%'"; 
                $sql .= "and BranchID like '%".$requestData['search']['value']."%'"; 
                $sql .= "or DivisionCode like '%".$requestData['search']['value']."%'";
                $sql .= "or DivisionName like '%".$requestData['search']['value']."%'";
                $sql .= "or DivisionAlias like '%".$requestData['search']['value']."%'"; 
            }
        ...
        foreach ($row as $row) {
            ...
            $nestedData[] = $no++;     
            $nestedData[] = $row["DivisionID"];     
            $nestedData[] = $row["BranchID"];
            $nestedData[] = $row["DivisionCode"];
            $nestedData[] = $row["DivisionName"];
            $nestedData[] = $row["DivisionAlias"];
    ...
    public function ajax_UpdateStatusCategory(){
        ...
        $id = trim(element('DivisionID',$i_list));
        ...
        $model = $this->global_m->ubah('Mst_Division', $data,'DivisionID',$id);
        if ($model) {
            $notifikasi = Array(
                'msgType' => true,
                'msgTitle' => 'Success',
    ...
    public function ajax_UpdateCategory(){
        ...
        $id_kyw=(int)$this->session->userdata('id_kyw');
        $BranchID = trim(element('BranchID',$i_list)); 
        $DivisionCode = trim(element('DivisionCode',$i_list));
        $DivisionName = element('DivisionName',$i_list); 
        $DivisionAlias = trim(element('DivisionAlias',$i_list));
        $iStatus = trim(element('Status',$i_list));
        
        if(element('DivisionID',$i_list)=="Generate"){
            $id = $this->master_division_m->getIdMax();
             $data = array(
            'DivisionID' => $id,
            'BranchID' => $BranchID,
            ...
            $id = trim(element('DivisionID',$i_list));
         $data = array(
            'DivisionID' => trim(element('DivisionID',$i_list)),
            'BranchID' => $BranchID,
            'DivisionCode' => $DivisionCode,
            'DivisionName' => $DivisionName,
            'DivisionAlias' => $DivisionAlias,
        ...
        if(element('DivisionID',$i_list)=="Generate"){
        $model = $this->global_m->simpan('Mst_Division', $data);
       }else{
        $model = $this->global_m->ubah('Mst_Division', $data,'DivisionID',$id);
       }
        if ($model) {
   ...
   public function getUserInfo() {
        ...
        $rows = $this->master_division_m->getUserInfo();
        $data['data'] = array();
        foreach ($rows as $row) {

            $array = array(
                                 
               'DivisionID' => trim($roq->DivisionID),    
               'BranchID' => trim($row->BranchID),
               'DivisionCode' => trim($row->DivisionCode),
               'DivisionName' => trim($row->DivisionName),
               'DivisionAlias' => trim($row->DivisionAlias) 
